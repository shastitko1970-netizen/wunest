...
Если нужен полноценный JS (state machines, API-вызовы) — это не поддерживается из соображений безопасности и не планируется. Используй документированный whitelist, а для state — CSS-only техники (`:checked ~ sibling`, `:target`, `<details>`) — они работают без доработок.
