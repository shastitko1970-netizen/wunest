...
### Размер и форма
- Размер шрифта чата (0.7× — 1.5×) — действует только на сообщения, не на шелл
- Ширина чата (40–100%)
- Форма аватара (круглые / квадратные / portrait)
- Стиль сообщений (пузыри / плоский / документ)
- **Скругление углов** (0.5× — 2×) — multiplier для всех `--nest-radius-*` (sm / base / lg). Пилюли (badges) не масштабируются — у них семантика «всегда максимально круглое»

### Палитра — поверхности и текст (M51)
- Фон страницы (`--nest-bg`)
- Карточки и панели (`--nest-surface` + `--nest-bg-elevated`) — single control для обеих, в дизайн-системе они визуально пара
- Основной текст (`--nest-text`)
- Вторичный текст (`--nest-text-secondary`) — без override автоматически derived через `color-mix(text 70% + bg 30%)`
- Приглушённый текст (`--nest-text-muted`) — без override `color-mix(text 45% + bg 55%)`
- Рамки (`--nest-border`)
- **Цвет иконок** (`--nest-icon-color`, M52.3) — отдельный control для всех декоративных Material Design icons (топбар, drawer'ы, диалоги). **Семантические иконки не затрагиваются**: красный «error», зелёный «success», жёлтый «warning» сохраняют свои значения через Vuetify color-классы. Без override иконки наследуют `currentColor` от родителя как раньше

Каждый цвет — нативный color picker + текстовое поле + кнопка ✕ для clear-override (вернуться к каскаду активной темы).

### Типографика (M51)
Один picker который меняет и body, и display-шрифты сразу:
- **Default (preset)** — по умолчанию шрифт активной темы
- **System UI** — системный шрифт ОС (SF / Segoe / Roboto)
- **Sans (Outfit)** — текущий default-sans
- **Serif (Fraunces)** — для длинных чтений
- **Mono (JetBrains Mono)** — code-vibe для всего интерфейса
- **Custom** — раскрывает поле для произвольного CSS font-family stack

Шрифт mono-блоков (code, kbd) намеренно не меняется — code-блоки не должны внезапно становиться засечками.

### Акцент и фон
- Акцентный цвет (`--nest-accent`)
- Фоновая картинка + blur (0–30)

### Поведение
- Тени, анимации, HTML-рендеринг
- **Следовать системной теме (M51)** — при смене системного dark/light (macOS, Windows, GNOME) автоматически переключаемся между парой preset’ов. Pair-логика: `cyber-neon ↔ minimal-reader`, `nest-default-dark ↔ nest-default-light`, у `tavern-warm` нет light-сиблинга — fallback на `nest-default-light`. Ручной выбор preset (через picker или галерею) выключает этот режим — явный выбор всегда побеждает. Default — выключено: opt-in, чтобы не было сюрпризов на первом paint'е.

Это пишет CSS-переменные на `:root` inline — срабатывает мгновенно без перезагрузки. Сохраняется на сервер с debounce 400ms — синкается между устройствами через Appearance store.
...
- `border_color` → автоматически промотируется ещё и в `accent` (теме нужен один акцент по умолчанию).
- `blur_tint_color` сейчас **не импортируется** — у нас нет соответствующего поля в Appearance. Не теряется при ре-экспорте, но и не применяется. (Backlog M51.)
- `avatar_style: 2` (portrait) корректно round-trip'ится через ST-export (`toST` сохраняет тройку round/square/portrait как 0/1/2).

#### JSON-схема (что можно класть в файл)

```jsonc
{
  "name": "My Theme",                       // string, optional, для UI-метки
  ...
  "border_color": "#2a3140",                // CSS color string (ещё промотируется в accent)
  "blur_tint_color": "#0f1217",             // ⚠️ принимается, но не применяется (backlog)

  "font_scale": 1.0,                        // 0.7…1.5, всё за пределами клампится
  "chat_width": 80,                         // 40…100 (% от viewport)
  "blur_strength": 6,                       // 0…30 (px)

  "avatar_style":  0,                       // 0=круг, 1=квадрат, 2=portrait
  "chat_display":  1,                       // 0=плоский, 1=пузыри, 2=документ

  "noShadows": false,                       // boolean, true = выкл. тени
  "reduced_motion": false,                  // boolean, true = выкл. анимации

  "custom_css": "..."                       // string, до ~256 KiB. Применяется как свой CSS.
}
```

Все поля опциональные — недостающие берут default'ы из активной темы. Числа за пределами диапазона **молча клампятся** (не reject'ятся). Невалидные цвета браузер сам игнорирует.

#### Лимит на `custom_css`

Сервер хранит весь Appearance-блок (включая `custom_css` + `bg_image_url` + остальные поля) одним JSON-объектом, **общий cap 256 KiB**. На практике это ~3000 строк CSS. Если упёрся — `@import` в текстовое поле тоже работает.
...
### LLM-конвертер ST → WuNest

Если ST-тема использует селекторы, которых нет в WuNest (например, `#expression-image`, `.drawer-content`), её можно прогнать через **/convert** — там LLM перепишет селекторы в WuNest-нативные (`.nest-msg`, `#chat`).

Что умеет конвертер (M51 Sprint 2):

- **Файл** или **вставленный текст** — оба варианта на тех же 500 KB
- **`.json`** (ST envelope) или **`.css`** (raw CSS) — авто-определение
- **Превью CSS** перед apply — первые 30 строк + счётчик переписанных селекторов
- **Retry с другой моделью** — кнопка «Попробовать другую модель» рядом с результатом и в каждой строке истории. Без re-upload — input хранится 24h
- Лимит **3 конвертации в час** на пользователя (retry считается)
- Тратит токены **твоего ключа** (BYOK или WuApi pool)
