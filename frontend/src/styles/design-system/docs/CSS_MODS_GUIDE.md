...
WuNest держит всю внешность на **CSS-переменных**. Твой CSS идёт в `<style id="nest-user-css">` в `<head>` и по умолчанию оборачивается в `@scope(#chat)` — всё, что ты написал, применяется **только внутри области чата**.
...
2. Найди в `<head>` `<style id="nest-user-css">` — видно, как именно CSS был скопирован.
...
3. Внутренняя галерея тем — `/themes`. Готовые пресеты + 5-color swatch на каждой карточке. Источник — `*.theme.json` манифесты в `frontend/src/styles/themes/`. Третья сторона может добавить свою тему: `THEME_PRESETS` строится через Vite glob — см. [`themes-authoring.md`](./themes-authoring.md).
